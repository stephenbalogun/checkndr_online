    ...
    out[[i]] <- switch(input[[i]],
                       ...
                       "Viral load result" = subset(
                         ...
                         recency_test_name %in% c("Asante", "AS") &
                           control_line %in% "Yes" & verification_line %in% "Yes" &
                           !longterm_line %in% "Yes" & viral_load_requested %in% "Yes" &
                           recency_test_date < max(recency_test_date, na.rm = TRUE) - days(28) & is.na(date_of_viral_load_result)
                       ),
                       "Partial duplicates" = get_dupes(df, sex, date_of_birth, facility, visit_date)
    )
  }
  names(out) <- input
